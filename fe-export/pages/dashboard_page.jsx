...
import { LogOut, Shield, Lock, Globe, Wifi, Activity, Zap, Radio, ArrowRight } from 'lucide-react';
    ...
    <div className="bg-surface-raised border border-slate-800/60 rounded-xl p-4 hover:border-slate-700/60 transition-all group">
      <div className="flex items-center justify-between mb-3">
        ...
        <div className="flex items-center gap-1.5">
          {id === 'tls' ? (
            <span className="text-[8px] font-semibold text-emerald-400 bg-emerald-500/10 px-1.5 py-0.5 rounded-full border border-emerald-500/20">✓ Live DB</span>
          ) : (
            <span className="text-[8px] font-semibold text-amber-400 bg-amber-500/10 px-1.5 py-0.5 rounded-full border border-amber-500/20">⚠ Simulated</span>
          )}
          {isLive && (
...
export default function DashboardPage() {
  ...
  if ((l1 || l2) && !pd && !cd) return <LoadingState label="Connecting to QUANSEC..." />;
